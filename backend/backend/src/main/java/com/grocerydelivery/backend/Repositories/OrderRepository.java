...
//    Long getOrderid(@Param("USERID") Long USERID);
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE ORDERDETAILS O JOIN TIMESLOT T ON O.TIMESLOTID=T.SLOTID SET O.ORDERSTATUSID=:ORDERSTATUSID WHERE O.ORDERID=:ORDERID AND (TIME(NOW()) BETWEEN T.STARTTIME AND T.ENDTIME) AND O.DELIVERYDATE=CURRENT_DATE()",nativeQuery = true)
